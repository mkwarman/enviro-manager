...
def index():
    return "<p><h1>Sensor Status:</h1>" \
            + "<h2>Probe temp: {0:0.2f} (target: {1}F) --- Last updated: {2}</br>".format(probe_temp, MAT_TEMP_TARGET, probe.last_updated) \
            + (("<strong>" + probe.disabled_string + "</strong>") if probe.disabled else "") \
            + "Sensor1 (temperature): temp={0:0.2f}F hum={1:0.2f}% (target temperature: {2}F) --- Last updated: {3}</br>".format(sensor_values[0].get('temp'), sensor_values[0].get('hum'), AMBIENT_TEMP_TARGET, dht1_temp.last_updated) \
            + (("<strong>" + dht1_temp.disabled_string + "</strong>") if dht1_temp.disabled else "") \
            + "Sensor2 (humidity): temp={0:0.2f}F hum={1:0.2f}% (target humidity range: {2}% to {3}%) --- Last updated: {4}</h2></p>".format(sensor_values[1].get('temp'), sensor_values[1].get('hum'), HUMIDITY_LOWER_BOUND, HUMIDITY_UPPER_BOUND, dht2_humidity.last_updated) \
