...
def index():
    return "<p><h1>Sensor Status:</h1>" \
            + "<h2>Probe temp: {0:0.2f} (target: {1}F) --- Last updated: {2:%H:%M:%S %Y-%m-%d}</br>".format(probe_temp, MAT_TEMP_TARGET, probe.last_updated or "never") \
            + (("<strong>" + probe.disabled_string + "</strong>") if probe.disabled else "") \
            + "Sensor1 (temperature): temp={0:0.2f}F hum={1:0.2f}% (target temperature: {2}F) --- Last updated: {3:%H:%M:%S %Y-%m-%d}</br>".format(sensor_values[0].get('temp'), sensor_values[0].get('hum'), AMBIENT_TEMP_TARGET, dht1_temp.last_updated or "never") \
            + (("<strong>" + dht1_temp.disabled_string + "</strong>") if dht1_temp.disabled else "") \
            + "Sensor2 (humidity): temp={0:0.2f}F hum={1:0.2f}% (target humidity range: {2}% to {3}%) --- Last updated: {4:%H:%M:%S %Y-%m-%d}</h2></p>".format(sensor_values[1].get('temp'), sensor_values[1].get('hum'), HUMIDITY_LOWER_BOUND, HUMIDITY_UPPER_BOUND, dht2_humidity.last_updated or "never") \
